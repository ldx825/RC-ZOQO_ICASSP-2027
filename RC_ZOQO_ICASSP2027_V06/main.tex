...
\section{Conclusion}
We identified a terminal resolution wall in quantized ZO: once the grid cannot express the intended motion, fixed-bit runs keep paying queries for updates that cannot carry signal. The wall has a computable price (curvature-weighted floor laws, approximately halved by each extra bit) and a testable state: the resolution audit turns the one-grid regime into a decision with an exact finite-sample certificate and a finite accepted-descent bound, stopping an exhausted full model ($89.3$\%/$33.8$\% fewer post-floor/total like-for-like queries, no accuracy loss) and continuing a head-only block, recovering $+1.14$ points a schedule stop discards.
