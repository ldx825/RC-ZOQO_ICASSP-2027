...
import json
root=Path(__file__).resolve().parents[1]; log=root/'outputs/logs'; out=root/'outputs/EXPERIMENT_REPORT.md'
def status(name):
 p=log/name
 if not p.exists(): return 'PARTIALLY SUPPORTED (not run)'
 d=json.loads(p.read_text()); return 'SUPPORTED' if d.get('passed',d.get('success_bound_checked',False)) else 'NOT SUPPORTED'
text=f'''# RC-ZOQO Validation Report
...
**{status("exp1_status.json")}** — deterministic two-cycle test.
...
**{status("exp3_status.json")}** — exact linear counterexample and random boundary sweep.

## 6. Range–Resolution Feasibility
**{status("exp4_status.json")}** — prediction compared with endpoint-inclusive brute-force search.
